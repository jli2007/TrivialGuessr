...
const GameResult: React.FC<GameResultProps> = ({
  ...
  const averageDistance = answers.reduce((acc, ans) => acc + (ans.distance || 0), 0) / answers.length;
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-900 via-purple-900 to-indigo-900 flex items-center justify-center p-4">
      <div className="max-w-4xl w-full bg-white/10 backdrop-blur-md rounded-xl p-8">
        <div className="text-center mb-6">
          ...
          <p className="text-white">Average Distance: {formatDistance(averageDistance)}</p>
        </div>
            ...
            <div className="space-y-3">
              {answers.map((answer, index) => (
                <div key={index} className="bg-white/10 rounded p-3">
                  <div className="text-white font-medium mb-1">Round {index + 1}</div>
                  <div className="text-sm text-blue-200 mb-2">{answer.correctAnswer}</div>
                  <div className="flex justify-between items-center">
                    <span className="text-green-400">{formatDistance(answer.distance || 0)}</span>
                    <span className="text-yellow-400 font-bold">+{answer.score.toLocaleString()}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white/20 rounded-lg p-4">
            ...
            <div className="space-y-2">
              <div className="flex justify-between">
                ...
                <span className="text-white">{answers.filter(a => a.distance !== null && a.distance < 1).length}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-blue-200">Close Guesses (&lt;100km):</span>
                <span className="text-white">{answers.filter(a => a.distance !== null && a.distance < 100 && a.distance >= 1).length}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-blue-200">Best Guess:</span>
                <span className="text-green-400">
                  {formatDistance(Math.min(...answers.map(a => a.distance || Infinity).filter(d => d !== Infinity)))}
                </span>
              </div>
